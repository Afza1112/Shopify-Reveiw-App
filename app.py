...
def submit_review():
    ...
    if 'image' in request.files and request.files['image'].filename != '':
        ...
        print("USER UPLOADED FILE:", file.filename)  # Debug print!
        if file and allowed_file(file.filename):
            ...
            file.save(save_path)
            print("IMAGE SAVED TO:", save_path)  # Debug print!
            image_url = url_for('static', filename='review_images/' + filename)
        else:
            print("File type not allowed!")  # Debug print!
    else:
...
def approve_review(review_id):
    result = reviews.update_one({"_id": ObjectId(review_id)}, {"$set": {"approved": True}})
